...
export = {
    ...
    enumWindowByClassName(className: string): number[] {
        const wins: string = dm.EnumWindow(undefined, undefined, className, 2)
        return wins.length > 0 ? wins.split(",").map(hWnd => Number(hWnd)) : []
    },
    ...
    setWindowTitle(hWnd: number, title: string): DmRet {
        return dm.SetWindowText(hWnd, title)
    },
    ...
    readInt(hwnd: number, addr: string, type: number): number {
        return dm.ReadInt(hwnd, addr, type)
    },
    readInt32(hwnd: number, addr: string): number {
        return this.readInt(hwnd, addr, 0)
    }
}
